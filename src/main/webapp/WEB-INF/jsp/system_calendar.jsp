...
<div class="container">
	<section id="customerList">
		...
		<div class="row">
			...
			<div class="col-lg-8">
				<button id="cmdCalendarAdd" type="submit"
					class="btn btn-primary pull-right border-custom"
					onclick="cmdCalendarAdd_OnClick()">Add</button>
			</div>
		</div>
		<br />
		<div class="row">
		...
		<div class="modal-content border-custom">
			...
			<div class="modal-body">
				<form id="messageForm">
					<dl class="dl-horizontal">
						...
						<dt>Daycode:</dt>
						<dd>
							<input class="form-control border-custom" id="EDIT_CLDR_DAYCODE"
								name="EDIT_CLDR_DAYCODE" type="text" required />
						</dd>
						<dt>Note:</dt>
						<dd>
							<input class="form-control border-custom" id="EDIT_CLDR_NOTE"
								name="EDIT_CLDR_NOTE" type="text" required />
						</dd>
					</dl>
				</form>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-primary border-custom"
					id="CmdCalendarEditOk" onclick="cmdCalendarEditOk_OnClick()">
					Ok</button>
				<button type="button" class="btn btn-danger border-custom"
					id="CmdCalendarEditCancel"
					onclick="cmdCalendarEditCancel_OnClick()">Cancel</button>
			</div>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	...
	function cmdCalendarEdit_OnClick() {
		...
		document.getElementById('EDIT_CLDR_ID').value = calendar.CLDR_ID !== null
				&& typeof (calendar.CLDR_ID) != 'undefined' ? wijmo.Globalize
				.format(calendar.CLDR_ID) : 0;
		...
		calendarDate = new wijmo.input.InputDate(
				'#EDIT_CLDR_DATE',
				{
					format : 'MM/dd/yyyy',
					value : new Date(splitDate[0], splitDate[1] - 1,
							splitDate[2]),
					mask : '99/99/9999',
					onValueChanged : function() {
						document.getElementById('EDIT_CLDR_DATE_DATA').value = this.value
								.toString("yyyy-MM-dd");
					}
				});
	}
	...
	function cmdCalendarAdd_OnClick() {
		...
		document.getElementById('EDIT_CLDR_DATE_DATA').value = currentDate
				.toString("yyyy-MM-dd");
						...
						function(e) {
							if (e) {
								$
										.ajax({
											type : "DELETE",
											url : '${pageContext.request.contextPath}/api/calendar/delete/'
													+ id,
	...
	function cmdCalendarEditOk_OnClick() {
		...
		calendarObject.CLDR_ID = parseInt(document
				.getElementById('EDIT_CLDR_ID').value);
		calendarObject.CLDR_DAYCODE = document
				.getElementById('EDIT_CLDR_DAYCODE').value;
		calendarObject.CLDR_NOTE = document.getElementById('EDIT_CLDR_NOTE').value;

		var splitDate = document.getElementById('EDIT_CLDR_DATE_DATA').value
				.split("-");

		calendarObject.CLDR_DATE = new Date(splitDate[0], splitDate[1] - 1,
				splitDate[2]);
	...
	function updateNavigateButtonsCalendar() {
		...
		btnCurrentPageGrid.innerHTML = (calendars.pageIndex + 1) + ' / '
				+ calendars.pageCount;
	...
	$(document)
			.ready(
					function() {
						...
						// Collection View
						calendars = new wijmo.collections.CollectionView(
								getCalendars());
						...
						calendars.filter = function(item) {
							return !filterText
									|| (item.CLDR_DAYCODE.toLowerCase()
											.indexOf(filterText) > -1);
						}

						calendars.collectionChanged.addHandler(function(sender,
								args) {
							updateNavigateButtonsCalendar();
						});

						calendars.currentChanged.addHandler(function(sender,
								args) {
							updateDetails();
						});
