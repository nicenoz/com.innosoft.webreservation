	...
	<section>
		<div class="container">
			<div class="row">
				...
				<div class="col-lg-2">
					<button id="cmdGetSchedule" type="submit"
						class="btn btn-primary border-custom btn-block pull-right"
						onclick="cmdGetSchedule_OnClick()">Get</button>
				</div>
				<div class="col-lg-2 pull-right">
					<button id="cmdCreateReservation" type="submit"
						class="btn btn-success border-custom btn-block"
						onclick="cmdAddReservation_OnClick()">Add</button>
				</div>
			</div>
			<br />
			<div class="row">
				<div class="col-lg-12">
					<div id="scheduleGrid" class="grid border-custom"
			...
			<div class="row">
				<div class="col-lg-12">
					<button class='btn btn-success btn-xs border-custom'>x</button>
					- Your Reservations &nbsp &nbsp &nbsp &nbsp &nbsp
					<button class='btn btn-primary btn-xs border-custom'>x</button>
					- Taken Reservations
				</div>
			</div>
		</div>
	...
	<div class="modal-dialog">
		<div class="modal-content border-custom">
				...
				<form id="messageForm">
					<dl class="dl-horizontal">
						...
						<dd>
							...
							<input type="text" class="form-control autocomplete-wide"
								id="AE_CUST_NAME" readonly>
						</dd>
						<dt>Calendar Date:</dt>
						<dd>
						...
						<dd>
							<textarea cols="*" rows="3" id="AE_RESV_NOTES"
								name="AE_RESV_NOTES"
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-danger border-custom pull-left"
					id="CmdDelete" onclick="cmdDelete_OnClick()">Delete</button>
				<button type="button" class="btn btn-primary border-custom"
					id="CmdAddEditOk" onclick="cmdAddEditOk_OnClick()">Ok</button>
...
<script type="text/javascript">
				...
				.ajax(
						{
							...
							data : {
								"customerId" : cboCustomer.selectedValue.id
							},
																...
																.forEach(function(
																	...
																	reservationsList
																			.push({
																				...
																				RESV_MEBR_NAME : reservation.RESV_MEBR_FK["MEBR_LAST_NAME"]
																						+ ", "
																						+ reservation.RESV_MEBR_FK["MEBR_FIRST_NAME"]
																			});
																	console
																			.log(reservation.RESV_ID);
																});
				...
				.ajax(
						{
							...
							success : function(results) {
								...
								if (results.length > 0) {
									for (i = 0; i < results.length; i++) {
										...
										customerTimeFlat
												.push({
													...
													time : results[i]["CTIM_DETAILS_NO"]
															.toString(),
												});
												...
												for (a = 0; a < reservationsList.length; a++) {
													...
													if (reservationsList[a].RESV_CUST_ID == customerId) {
														...
														if (calendarActivities[k].id == reservationsList[a].RESV_CACT_ID) {
															...
															if ((results[i]["CTIM_ID"] >= reservationsList[a]["RESV_START_TIME_ID"])
																	&& ((results[i]["CTIM_ID"] <= reservationsList[a]["RESV_END_TIME_ID"]))) {
																...
																if (reservationsList[a]["RESV_PARTS_NO"] == (p + 1)) {
																	...
																	if (reservationsList[a]["RESV_MEBR_ID"] == loggedInCustomerId) {
																		...
																		slotHolder = slotHolder
																				+ "<button class='btn btn-success btn-xs border-custom' onclick='cmdEditReservation_OnClick(\""
																				+ a
																				+ "\", true)'>x</button> ";
																	} else {
																		//VIEW ONLY IF NOT
																		slotHolder = slotHolder
																				+ "<button class='btn btn-primary btn-xs border-custom' onclick='cmdEditReservation_OnClick(\""
																				+ a
																				+ "\", false)'>x</button> ";
																	}
																}
									...
									scheduleCollection = new wijmo.collections.CollectionView(
											customerTime);
									scheduleCollection.canFilter = true;

									scheduleGrid.dispose();
									scheduleGrid = new wijmo.grid.FlexGrid(
											'#scheduleGrid');
	...
	function cmdAddEditOk_OnClick() {
		...
		reservation.RESV_CUST_ID = parseInt(document
				.getElementById('AE_CUST_ID').value);
		reservation.RESV_MEBR_ID = parseInt(document
				.getElementById('AE_MEBR_ID').value);
		reservation.RESV_CACT_ID = parseInt(document
				.getElementById('AE_CACT_ID').value);
		reservation.RESV_UNIT_NO = /* parseInt(document.getElementById('AE_UNIT_NO').value) */1;
		reservation.RESV_PARTS_NO = parseInt(document
				.getElementById('AE_PARTS_NO').value);
		reservation.RESV_START_TIME_ID = parseInt(document
				.getElementById('AE_START_TIME_ID').value);
		reservation.RESV_END_TIME_ID = parseInt(document
				.getElementById('AE_END_TIME_ID').value);
					...
					success : function(data) {
						if (data.RESV_ID > 0) {
							if (reservation.RESV_ID == 0) {
								...
								emailObject.EMAIL_SUBJECT = cboCustomer.selectedValue.customerName
										+ " Reservation";
								emailObject.EMAIL_MESSAGE = "Reservation Date: "
										+ cboAECalenderDate.selectedValue
										+ "\nPart No.: "
										+ document
												.getElementById('AE_PARTS_NO').value
										+ "\nTime Start: "
										+ cboAEStartTime.selectedValue
										+ "\nTime End: "
										+ cboAEEndTime.selectedValue
										+ "\nNote: "
										+ document
												.getElementById('AE_RESV_NOTES').value;
								...
								toastr.success(getMessage("E0006"));
							}
						...
						function(e) {
							if (e) {
								$
										.ajax({
											type : "DELETE",
											url : '${pageContext.request.contextPath}/api/reservation/delete/'
													+ currentLookup,
											contentType : "application/json; charset=utf-8",
											dataType : "json",
											statusCode : {
	...
	function cmdAddReservation_OnClick() {
		if (isScheduleUpdated) {
			...
			cboAECalenderDate = new wijmo.input.ComboBox(
					'#AE_CALENDAR_DATE',
					{
						itemsSource : calendarActivityList.slice(
								startDateIndex, endDateIndex + 1),
						isEditable : false,
						onSelectedIndexChanged : function() {
							document.getElementById("AE_CACT_ID").value = calendarActivities[cboAECalenderDate.selectedIndex
									+ startDateIndex].id;
						}
					});
			...
			cboAEPartsNo = new wijmo.input.ComboBox(
					'#AE_PARTS',
					{
			...
			cboAEStartTime = new wijmo.input.ComboBox(
					'#AE_START_TIME',
					{
			...
			cboAEEndTime = new wijmo.input.ComboBox(
					'#AE_END_TIME',
					{
						itemsSource : customerTimeList,
						isEditable : false,
	...
	function cmdEditReservation_OnClick(customerId, isUser) {
		...
		if (isScheduleUpdated) {
			...
			for (a = 0; a < calendarActivities.slice(startDateIndex,
					endDateIndex + 1).length; a++) {
				console.log(calendarActivities.slice(startDateIndex,
						endDateIndex + 1)[a].id
						+ " " + reservation.RESV_CACT_ID)
	...
	function createCboCalendarActivity(calendarActivities) {
		calendarActivityCollection = new wijmo.collections.CollectionView(
				calendarActivities);

		calendarActivityList = new Array();
		for (var i = 0; i < calendarActivityCollection.items.length; i++) {
			calendarActivityList
					.push(calendarActivityCollection.items[i].dayCode);
		}

		cboCalendarActivityStart.dispose();
		cboCalendarActivityStart = new wijmo.input.ComboBox(
				'#cboCalendarActivityStart',
				{
		...
		cboCalendarActivityEnd = new wijmo.input.ComboBox(
				'#cboCalendarActivityEnd',
				{
					itemsSource : calendarActivityList,
					isEditable : false,
					onSelectedIndexChanged : function() {
	...
	$(document)
			.ready(
					function() {
						...
						cboCustomer = new wijmo.input.AutoComplete(
								'#cboCustomer');
						cboCalendarActivityStart = new wijmo.input.AutoComplete(
								'#cboCalendarActivityStart');
						cboCalendarActivityEnd = new wijmo.input.AutoComplete(
								'#cboCalendarActivityEnd');

						cboAECalenderDate = new wijmo.input.AutoComplete(
								'#AE_CALENDAR_DATE');
						cboAEPartsNo = new wijmo.input.AutoComplete('#AE_PARTS');
						cboAEStartTime = new wijmo.input.AutoComplete(
								'#AE_START_TIME');
						cboAEEndTime = new wijmo.input.AutoComplete(
								'#AE_END_TIME');
